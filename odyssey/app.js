/* CKDesign — collection display engine ("The Odyssey").
   ...
   This room: cool cave grays, masthead top-RIGHT (mirrors the Gorgon room). */
...
const COLLECTION = {
  ...
  /* stone / acrylic-wash finish (same hand-finished look as the Gorgons) */
  finish: {
...
function layout() {
  ...
  if (window.parent !== window && postH !== lastPostedH) {
    ...
    try { parent.postMessage({ ckdCol: 'odyssey', h: postH }, '*'); } catch (_) {}
  }
