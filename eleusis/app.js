/* CKDesign — collection display engine ("The Eleusinian Mysteries").
   ...
   This room: a very-slight-pink wall, masthead top-LEFT, a perfect row of 3. */
...
const COLLECTION = {
  ...
  /* a VERY SLIGHT pink wall — sacred, initiatory, softer than the other two
     rooms (Gorgon warm tan, Odyssey cool cave gray) */
  backdrop: {
    center:'#f6eef1', edge:'#e6d8dd',      // barely-pink light stone
    vein:'#c9b4bd', veinAmount:0.15,
    grain:9, seed:31,
    lightX:0.5, lightY:0.38,
  },
  /* stone / acrylic-wash finish (same hand-finished look as the other rooms) */
  finish: {
    ...
    clearcoat:0.06, sheen:0.22, sheenColor:0xf3ecef,
    veinAmount:0.10, veinScale:2.6,
    env:0.5,
    hoverTint:0xfff1f5,
  },
  /* landscape scatter — a PERFECT ROW: same size (s), same depth (z:0), same
...
const ASSET_V = 1;   // bump after every model reconversion — GLBs cache like scripts do
...
function layout() {
  ...
  if (fontsReady && window.parent !== window && Math.abs(postH - lastPostedH) > 8) {
    ...
    /* ckdCol (NOT ckd) — the /design/ page hosts BOTH collection iframes, and
       ...
       A distinct key per collection stops the modules resizing each other. */
    try { parent.postMessage({ ckdCol: 'eleusis', h: postH }, '*'); } catch (_) {}
  }
